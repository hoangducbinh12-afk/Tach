...
st.markdown("""
    ...
    .dan-box-1 { background-color: #e8f5e9; padding: 10px; border-radius: 5px; color: #2e7d32; font-family: monospace; font-size: 13px; font-weight: bold; border: 1px solid #c8e6c9; }
    .dan-box-2 { background-color: #e3f2fd; padding: 10px; border-radius: 5px; color: #1565c0; font-family: monospace; font-size: 13px; font-weight: bold; border: 1px solid #bbdefb; }
...
# --- 5. UI ---
st.markdown("<div class='main-title'>💎 18 BIEN PRO - FULL OPTION</div>", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    show_kh("CHẴN LẺ 4", "cl4", list(CL_4.keys())); show_kh("BÉ TO 4", "bt4", list(BT_4.keys()))
...
with tabs[2]:
    ...
    for r_name, r_val in [("Ngày", rd), ("Kỳ", rk), ("GĐB", rg)]:
        if r_val in ROOT_DATA:
            ...
            df_rt = pd.DataFrame(ROOT_DATA[r_val]).T
            df_rt.columns = range(10)
            st.table(df_rt)
