...
st.set_page_config(page_title="18 BIEN PRO - NEW", layout="centered")
...
# --- 3. DỮ LIỆU GỐC (ĐÃ FIX TÊN BIẾN ĐỂ TRÁNH NAMEERROR) ---
...
with tabs[0]:
    ...
    with cn1:
        ...
        st.success(d1)
        if st.button("📋 Copy Dàn 1"): st.write(f'<script>navigator.clipboard.writeText("{d1}")</script>', unsafe_allow_html=True); st.toast("Copied D1")
    with cn2:
        st.session_state.n2 = st.number_input("Dàn 2:", 1, 100, st.session_state.n2)
        d2 = ", ".join(df_final.head(st.session_state.n2)["s"].tolist())
        st.info(d2)
        if st.button("📋 Copy Dàn 2"): st.write(f'<script>navigator.clipboard.writeText("{d2}")</script>', unsafe_allow_html=True); st.toast("Copied D2")
